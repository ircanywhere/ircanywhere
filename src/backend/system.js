 ...
 * Setup syslogs.
 */
exports.System.setupSyslog = function()
{
	var _this = this;
		_this.inputValues = {};
	...
	{
		_this.inputValues[_this.inputs[input]] = true;
	}

	// setup valid inputs

	syslog.init('web-bouncer', syslog.LOG_PID | syslog.LOGODELAY, syslog.LOG_LOCAL0);
	// setup syslog

	_this.log.sys = function(input, data)
	{
		if (!(input in _this.inputValues))
		{
			return false;
		}
		// invalid input
		...
		{
			//syslog.log(syslog.LOG_INFO, input + ' ' + JSON.stringify(data));
		}
		else
		{
			//syslog.log(syslog.LOG_INFO, input + ' ' + data);
		}
		// is it json or a string
	};
	// setup loggly with our config
};
