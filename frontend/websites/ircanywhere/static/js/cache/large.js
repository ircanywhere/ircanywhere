...
null:f.isFunction(a[b])?a[b]():a[b]},o=function(){throw Error('A "url" property or function must be specified');}}).call(this);var Templates = {
	...
	messageRow: function(action)
	{
		if (action)
		{
			return '<div class="clear row${cssClass}" data-type="privmsg" data-id="${id}" data-self="${self}" data-read="${read}" data-highlight="${highlight}" data-time="${date}">' +
				'<span class="column3 time">${time}</span>' +
	...
	reOrganiseChannelEvents: function(tab, time, me)
	{
		...
		if (_this.collapseableTypes.indexOf(prev.attr('data-type')) == -1 && _this.collapseableTypes.indexOf(me.attr('data-type')) > -1)
		{
			...
			$.when(el.insertBefore(me)).done(callback);
		}
	...
	connect: function(data, connect)
	{
		...
		if (data != null)
		{
			this.node_information = data.node_information;
		...
		{
			if (!this.node_information.running || this.node_information == null)
				this.onClose();
			// node is offline

			this.socket = io.connect(this.node_information.endpoint, {
				'secure': true,
				'port': 443,
				'reconnect': false,
				'auto connect': true,
	...
	onBackLog: function(data)
	{
		...
		$.get(this.node_information.endpoint + data.url, function(data)
		{
	...
	onChanList: function(data)
	{
		...
		$.get(this.node_information.endpoint + odata.url, function(data)
		{
...
var client = new SocketEngine();var Workspace = Backbone.Router.extend({
	...
	addNetwork: function()
	{
		...
		{
			this.loadStatic('/static/html/network.html', true, function()
			{
				$('div#footer div.link').addClass('selected');
				tabCollections.changeTopicBar('Add Network', '', '', '');
				$('div#home-content div#edit-warning').empty().remove();
	...
	editNetwork: function(network)
	{
		...
		this.loadStatic('/static/html/network.html', true, function()
		{
			...
			$('div#home-content div.clearfix, div#edit-warning').show();
			// clear the network form and update modal info

			if (!userInfo.account_type.canRemove && networkData.locked)
			{
				$('div#home-content form#network-form div.clearfix, div#edit-warning').hide();
				$('div#home-content div#connect-error').empty().html('<p>You are on a free account and this network is locked which means you cannot edit it.</p>').show();
				$('div#home-content input[type=submit]').hide();
			}
			// form is locked
			else
			{
...
var appRouter = new Workspace;var InputView = Backbone.View.extend({
	...
	onKeyDown: function(e)
	{
		...
		else if (keyCode == key.tab)
		{
			var users = tab.get('ulCollection').models,
				...
				currentInput = currentInputs.pop().replace(',', '').trim();
			...
			if (newId < newInputs.length)
			{
				...
				{
					this.setValue(newInputs[newId - 1] + ', ');
				}
				else
				{
					defaultInputs.pop();
					...
					var firstPart = (defaultInputs.length > 0) ? defaultInputs.join(' ') + ' ' : '',
						secondPart = (defaultInputs.length == 1) ? ', ' : '';
	...
	modalFormSubmit: function(e)
	{
		...
		else if ($('input#submit-type').val() == 'edit-network' && !(!userInfo.account_type.canRemove && userInfo.networks[editNet].locked))
		{
...
$(document).ready(function() {
	...
	reDraw: function(soft)
	{
		...
		if (this.tab.get('type') == 'chan' && !this.tab.get('hide_userlist'))
		{
			...
			if (this.tab.get('scrollPosition') - this.tab.$msgs.height() <= 0)
			{
				var _this = this;
				setTimeout(function() {
					_this.tab.$msgs.scrollTop(_this.tab.$msgs.find('.content').height());
				}, 500);
			}
		}
	...
	housekeep: function()
	{
		...
		else if (!this.ctcp)
		{
			...
			{
				this.tabId = actions.createWindow(this.options.network, tabPart, 'query', {bottomMessageId: this._id, finishedPlayback: false});
				this.tab = tabCollections.getByCid(this.tabId);
				this.pause = true;
				forcePush = true;
			}
	...
	render: function()
	{
		...
		this.$el = $.tmpl(template, {
			...
			time: $.generateTime(this.time, false),
	...
	render: function()
	{
		...
		this.$el = $.tmpl('noticeRow', {
			...
			time: $.generateTime(this.options.time, false),
			read: this.options.read,
			date: this.time
		});
	...
	renderNormal: function()
	{
		...
		else
		{
			...
			$.when(this.$el.insertAfter(selector), {el: selector}).then(function(el, obj)
			{
				...
				//Helper.generateCollapsedHeading(_this.tab, _this.tab.$msgs.find('div.collapsed-head:last span.event'), obj.el.next().attr('data-type'), obj.el.next());
	...
	playbackHandler: function()
	{
		...
		this.$msgs.debounce('scroll', function(e)
		{
			...
			if ((this.get('type') == 'chan' || this.get('type') == 'query' || this.get('type') == 'window') && this.get('scrollLock') === false)
			{
				...
				this.set({scrollLock: true, scrollPosition: -10, finishedPlayback: false});
...
var parser = new Parser();EventHandler = Backbone.Model.extend({
	...
	listEnd: function(network, buffer)
	{
		...
		tab.$link.find('a:first span.link').removeClass('net-loader').addClass('net-loaded');
		tab.$table.tablesorter().parent().css('marginRight', '-19px');
...
$.fn.isAtBottom = function()
{
	var inner = $(this).find('div.mcontainer'),
		...
		actualHeight = (inner[0].scrollHeight - inner.height() - row.outerHeight()),
		difference = actualHeight - inner.scrollTop();
		// some variables

	var eval1 = (actualHeight == inner.scrollTop()),
		eval2 = (difference >= -2 && difference <= 2),
		eval3 = (actualHeight < 0);
		// evaluates

	return (eval1 || eval2 || eval3);
	// minus the previous row height for all the padding and stuff
};
 ...
 *
 * Generate a a 00:00:00 from a timestamp
 */
...
$(window).load(function()
{
	...
	{
		if (data.logged_in && data.node_information == null)
		{
			client.hideLoading();
			client.onClose(true);
		}
