...
function NetworkManager() {
	...
	application.ee.on('pre-ready', self.preInit.bind(self));
...
/**
 * Called before every init in the application as a priority to gather information
 * about the clients from the database before they start getting modified by
 * everything else.
 *
 * @method preInit
 * @return void
 */
NetworkManager.prototype.preInit = function() {
	var networks = application.db.find('networks', {}),
...
NetworkManager.prototype.addNetwork = function(user, network, status) {
	...
	application.db.find('networks', {'internal.userId': new mongo.ObjectID(user._id), url: network.url}).count(function(err, docs) {
		...
		application.db.insert('networks', network, function(err, docs) {
...
NetworkManager.prototype.editNetwork = function(user, network) {
	...
	application.db.update('networks', {'_id': new mongo.ObjectID(network._id)}, _.omit(network, '_id'), {multi: false}, function(err) {
		if (err) {
			deferred.reject();
			return;
		}
